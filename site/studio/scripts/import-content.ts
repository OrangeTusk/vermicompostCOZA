...
await client.createOrReplace({
  ...
  featuredProducts: ['sifted-vermicompost', 'red-wiggler-worms', 'vermicompost-tea'].map((slug) => ({ _key: key(slug), _type: 'reference', _ref: `product-${slug}` })),
  ...
  valueImage: await uploadImage('/farm/windrows.jpg', 'Vermicomposting rows'),
